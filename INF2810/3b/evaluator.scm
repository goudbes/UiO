...
(define (eval-special-form exp env)
  (cond ((quoted? exp) (text-of-quotation exp))
        ...
        ((if? exp) (eval-if (cdr exp) env))
        ((and? exp) (eval-and (cdr exp) env));;Oppg 3a
        ((or? exp) (eval-or (cdr exp) env));;Oppg 3a
        ((lambda? exp)
         (make-procedure (lambda-parameters exp)
                         (lambda-body exp)
                         env))
        ((let? exp) (let->lambda (cdr exp) env));;Oppg3c&d
        ((while? exp) (eval-while exp env));;Oppg3e
        ((begin? exp) 
...
(define (eval-if exp env)
  (if (true? (mc-eval (if-predicate exp) env))
      ...
      (cond ((eq? (if-else? exp) 'else) (mc-eval (if-alternative exp) env))
            ((eq? (if-else? exp) 'elsif) (eval-if (if-elseif exp) env)))))
           
;;Oppgave 3e
(define (while? exp) (tagged-list? exp 'while))

(define (eval-while exp env)
  (define (while-test exp-1 body)
    (if (eq? ((mc-eval exp-1 env)) #t)
        (begin
          (mc-eval body env)
          (while-test exp-1 body))))
  (while-test (car exp) (cdr exp)))
...
(define (if-elseif exp) (cddddr exp))
...
(define (let->lambda exp env)
  (define lm '())
  (define var-list (list (car exp)))
  (define exp-list (list (caddr exp)))
  (define (make-let exp var-list exp-list)
    (if (eq? (car exp) 'and)
        (make-let (cddddr exp)
                  (append var-list (cons (cadr exp) '()))
                  (append exp-list (cons (cadddr exp) '())))
        (begin
          (set! lm (list (make-lambda var-list (cdr exp))))
          (set-cdr! lm exp-list)
          (mc-eval lm env))))
  (make-let (cdddr exp) var-list exp-list))
