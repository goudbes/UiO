...
public class LogFile {
  ...
  */
  ...
  public void noteNotTypeCheck(types.Type t1, String op,
                               types.Type t2, PascalSyntax where) {
    if (doLogTypeChecks)
      writeLogLine("Type check " + op + " on line " + where.lineNum +
              ": " + t1.identify() + " != " + t2.identify());
  }

  public void noteBinding(String id, PascalSyntax where, PascalDecl decl) {
     if (doLogBinding) {
       String declInfo = decl.identify().replaceFirst("on line 0", "in the library");
       writeLogLine("Binding on line " + where.lineNum + ": "
                    + id + " was declared as " + declInfo);
     }
  }
